...
#ifndef MYSLAM_LOGGING_H
#define MYSLAM_LOGGING_H
...
#define LOG_EVERY(severity, user_tag, count, ...)                        \
  {                                                            \
    static int _log_local_print_cnt = 0;                       \
    _log_local_print_cnt = (_log_local_print_cnt + 1) % count; \
    if (!_log_local_print_cnt) {                               \
      SLAM_LOG_FUNC(user_tag, severity, __VA_ARGS__);                    \
    }                                                          \
  }

//#ifdef ENABLE_DEBUG_LOG
//#define LOGD_EVERY(count, ...) LOG_EVERY(SLAM_DEBUG, count, __VA_ARGS__)
//#else
//#define LOGD_EVERY(...)
//#endif
//#ifdef ENABLE_INFO_LOG
//#define LOGI_EVERY(count, ...) LOG_EVERY(SLAM_INFO, count, __VA_ARGS__)
//#else
//#define LOGI_EVERY(...)
//#endif
//#ifdef ENABLE_ERROR_LOG
//#define LOGE_EVERY(count, ...) LOG_EVERY(SLAM_ERROR, count, __VA_ARGS__)
//#else
//#define LOGE_EVERY(...)
//#endif
